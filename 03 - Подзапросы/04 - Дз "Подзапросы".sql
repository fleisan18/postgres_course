-- 1. Вывести продукты,
-- количество которых в продаже меньше самого малого среднего количества продуктов в деталях заказов (группировка по product_id). 
...
-- 2. Напишите запрос, который выводит общую сумму фрахтов заказов для компаний-заказчиков
-- для заказов, стоимость фрахта которых больше или равна средней величине стоимости фрахта всех заказов,
...
FROM orders o
JOIN (SELECT AVG (freight) AS freight_avg, customer_id
     FROM orders
     GROUP BY customer_id) oa
USING (customer_id)
WHERE shipped_date BETWEEN '1996-07-16' AND '1996-07-31' AND freight >= oa.freight_avg
...
FROM products
JOIN (SELECT product_id, quantity
      FROM order_details
      WHERE quantity = 10) od
USING (product_id);
